...
export default {
  ...
  methods: {
    ...
    saveNewBoard() {
boardService.AddBoard(newBoard).then(response => { // 
  if(response.status === 201) { // if the post was successful (HTTP status 201)
  ...
  this.newBoard = {             // reset the newBoard object since we are done w/ it
            title ="",          // title to empty string
