<template>
  ...
  >Toggle Form
    ...
    <!--             only want to allow a submit when we have a valid form                 -->
    <!--    not allow submit to be active until we've determined we have a valid form      -->
    ...
    <!--         When it is clicked do nothing & reset the form              -->
    ...
    <!--       using v-bind:class={ classname : variable } ...next 5 rows    -->
    <div
      class="review"
...
export default {
  ...
  computed: { // computed properties are generated whenever data changes on screen
    isFormValid(){ // Determin if all the form fields have values - return true if they do, false if they don't
return this.newReview.reviewer    // true if reviewer is not empty (truthy)
    ...
    averageRating() {
      ...
      return (sum / this.reviews.length).toFixed(1); // calculate the average & round to 1 decimal place
    },
    numberOfOneStarReviews() {
      // call the numberOfReviews method passing it the array to search 
      //          & the number of stars we want it to count
  ...
  methods: {  // methods are run when they are called/referenced
    // method to receive & array & a rating number
    ...
    //  two parameters a value to hold the result & an element
    //  an initial value for the result is optional (but recommended)
    //  each time the function is called the result is updated & returned based
    //  on the elment
    ...
    numberOfReviews(reviews, starType) {
      return reviews.reduce((currentCount, review) => {
        ...
        //     & 1 to cuurent count, otherwise & 0 to current count
