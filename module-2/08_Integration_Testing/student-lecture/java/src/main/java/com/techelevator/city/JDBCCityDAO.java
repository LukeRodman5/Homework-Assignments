...
public class JDBCCityDAO implements CityDAO {
	...
	public void save(City newCity) {
		/* Since all the table columns are required, we need to be sure that all the variables in the City object
		 * passed to us are not null before we try to insert the data into the database
		 * If any of the values in the city object passed to us are null - throw an exception w/ a message
	...
	private long getNextCityId() {
		...
		if (nextIdResult.next()) {				// if a value was returned from the database manager
			return nextIdResult.getLong(1);		// retrieve from SQLRowSer result & return to caller
			...
			*					continue execution, (3) display an error message & terminate the program, or (4) throw an exception 
			*					which will terminate the program w/ the stack trace & error messages generated by the system
			* We chose to throw an exception this time because we want the stack trace displayed so we can see how we got the error
			*
			* RuntimeException is the super class of all exceptions - a generic exception
			* 
			* Throw a RuntimeException if you don't have a custom exceptions of system exception for what is wrong
			* 
			* When you throw an exception you may specify a message to be included w/ the system output displayed for the exception
			* 		to indicate what application error & why you threw the exceptions
			*
			*/
