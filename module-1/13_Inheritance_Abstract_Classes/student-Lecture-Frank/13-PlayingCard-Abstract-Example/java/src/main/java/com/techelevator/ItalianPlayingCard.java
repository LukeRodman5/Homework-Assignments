...
public class ItalianPlayingCard extends PlayingCard {
	...
	public ItalianPlayingCard() 
	{
		...
		initializeMaps(); // initializeMaps() will be run when this constructor is run even if the Maps are alrdy initialized
	} 

	public ItalianPlayingCard(int value, String suit) {
		super(value, suit, suitMap.get(suit));{ // potential error - this will fail if the Maps are not initialized
	
		if (value > MAXVALUE) {
			value = MAXVALUE; // set the current objects called value to MAXVALUE
		}
		if (value < MINVALUE) {
			setValue(MINVALUE);
		}
