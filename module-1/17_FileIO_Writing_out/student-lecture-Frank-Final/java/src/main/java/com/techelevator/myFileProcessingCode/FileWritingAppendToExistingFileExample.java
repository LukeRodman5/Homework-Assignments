...
public class FileWritingAppendToExistingFileExample {
	 ...
	 *                over PrintWriter & it's ability to append to an existing file
	 * @throws IOException 
	 *********************************************************************************/
	...
	public static void main(String[] args) throws IOException {  // IOExcpetion is super class of all I/O Exceptions
		...
		if (outputFile.exists()) { // if the file exists
			String choiceLetter = ""; 
			boolean isGoodResponse = false; // switch to hold whether or not we got a good response from the user
			do { // loop at least once while the condition at the end of the loop is true
				...
				choiceLetter = userResponse.substring(0,1).toUpperCase(); // Take first letter of the response & convert it to uppercase
				switch (choiceLetter) { 
					case "A": 
						appendToFile = true;
						isGoodResponse = true;
					...
					case "R":
						...
						isGoodResponse = true;
						break;
					default:			 // If not response expected, it's not a good response
		...
		// At this point in the program we have an output file defined on the disk & whether it shold be replaced or appended
		...
		// Define a PrintWriter for the BuffferedWriter so we can use the same methods as we did for System.out
		PrintWriter diskFileWriter = new PrintWriter(aBufferedWriter);
		
		
		// Get a String format for the current time-stamp to write to the file

        Timestamp timestampNow = Timestamp.valueOf(LocalDateTime.now());   // Get the current time-stamp
        ...
        timestampNow = Timestamp.valueOf(LocalDateTime.now());    // Get the current time-stamp
        diskFileWriter.println("--------------------------------------------------");
