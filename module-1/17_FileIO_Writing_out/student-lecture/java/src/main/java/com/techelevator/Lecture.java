...
public class Lecture {
	...
	public static void main(String[] args) throws IOException {
		...
		/* 
		 * TLDR: When you have a File object instantiated you can do a lot of stuff w/ it
		 * 
		 * The File class provides many methods to allow you to access the file system of your computer
		 * 
		 * Does a file exist?
		 * How many characters are in the file? (0=empty)
		 *  Create new files
		 *  Create new directories
		 *  Remove/delete files & directories
		 *  
		 *  Many of the things you can do w/ command line, you do in a program w/ a File object
		 * 
		...
		File newFile = new File(newDirectory, fileName);
									...
									// if directory is omitted, we assure the default directory
		...
		try(PrintWriter writer = new PrintWriter(newFile)) {// Putting the PrintWriter instantiation in a try so we don't 
