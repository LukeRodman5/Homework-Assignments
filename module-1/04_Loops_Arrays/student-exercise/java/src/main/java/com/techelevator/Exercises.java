...
public class Exercises {
	 ...
	 */
	public boolean firstLast6(int[] nums) {if (nums[0] == 6 || nums[nums.length-1] == 6) { // receives an array of ints called nums
		return true;
	}
		return false;
	}
	//look first at the last element in the array & if its 6 return true, otherwise false
	...
	/*
	 ...
	 makePi() → [3, 1, 4]
	 */
	public int[] makePi() { 		// receive no parameters & returns an int array
		return new int[] {3,1,4};	// int[] result = {3,1,4};
			}
	/*
	...
	public int[] reverse3(int[] nums) {
		...
		for (int i = nums.length - 1, j = 0; i >= 0; i--, j++) {  // i starts at the end of nums & decreases
			newArray[j] = nums[i];								  // j starts at the 0 & increases
		}
			return newArray;
			}
